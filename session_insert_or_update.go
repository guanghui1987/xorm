...
func (session *Session) innerInsertOrUpdateMulti(updateCols []string, rowsSlicePtr interface{}) (int64, error) {
	...
	if session.engine.dialect.DBType() == core.ORACLE {
		...
		if session.engine.dialect.DBType() == core.MYSQL {
			sql = fmt.Sprintf("INSERT INTO %s (%v%v%v) VALUES (%v) ON DUPLICATE KEY UPDATE %v",
				session.engine.Quote(tableName),
				session.engine.QuoteStr(),
				strings.Join(colNames, session.engine.QuoteStr()+", "+session.engine.QuoteStr()),
				session.engine.QuoteStr(),
	...
	session.cacheInsertOrUpdate(tableName)
...
func (session *Session) innerInsertOrUpdate(updateCols []string, bean interface{}) (int64, error) {
	...
	colNames, args, err := session.genInsertOrUpdateColumns(bean)
	if err != nil {
		return 0, err
	}
	...
	if len(colPlaces) > 0 {
		if session.engine.dialect.DBType() == core.MYSQL {
			sqlStr = fmt.Sprintf("INSERT INTO %s (%v%v%v) VALUES (%v) ON DUPLICATE KEY UPDATE %v",
				session.engine.Quote(tableName),
				session.engine.QuoteStr(),
		...
		if session.engine.dialect.DBType() == core.MYSQL {
			sqlStr = fmt.Sprintf("INSERT INTO %s VALUES () ON DUPLICATE KEY UPDATE %v",
				session.engine.Quote(tableName),
				strings.Join(updates, ","))
		} else {
...
// genInsertOrUpdateColumns generates insert needed columns
func (session *Session) genInsertOrUpdateColumns(bean interface{}) ([]string, []interface{}, error) {
	table := session.statement.RefTable
